...
export default function WeeklyReportPage() {
  ...
  const rows = report?.userStats ?? [];

  return (
    <div className="space-y-6">
      <PageHeader
        title="Weekly Report"
        description="Team performance summary for the selected week."
      />
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-sm font-medium min-w-[200px] text-center">{weekLabel}</div>
        <Button
          variant="outline"
          size="icon"
          onClick={() => setWeekOffset((o) => o + 1)}
      ...
      {isError && (
        <ErrorState title="Failed to load weekly report" description="Please try again." />
      )}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-right">Updates Submitted</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {rows.map((row: any) => (
