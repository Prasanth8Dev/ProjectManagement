...
export async function createBug(data: CreateBugInput): Promise<Bug> {
  return api.post('/bugs', data, { params: actorParams() });
}

export async function updateBug(id: string, data: UpdateBugInput): Promise<Bug> {
  return api.patch(`/bugs/${id}`, data, { params: actorParams() });
}
...
export async function assignBug(id: string, assigneeId: string | null): Promise<Bug> {
  return api.patch(`/bugs/${id}/assign`, { assigneeId }, { params: actorParams() });
}

export async function changeBugStatus(id: string, status: BugStatus): Promise<Bug> {
  return api.patch(`/bugs/${id}/status`, { status }, { params: actorParams() });
}

export async function convertBugToTask(id: string): Promise<Bug> {
  return api.post(`/bugs/${id}/convert-to-task`, undefined, { params: actorParams() });
}
